...
class Assignment( Statement ):
	...
	def tipe(self, tm):
		...
		if self.identifier not in tm:
			...
			print (self.identifier,tp)
		elif tm[self.identifier] != tp:
			error("Type Error: " + tm[self.identifier] + " = " + tp + "!")
...
class WhileStatement( Statement ):
	...
	def tipe(self, tm):
		if self.expression.tipe(tm) != "boolean":
			error("Type Error: While expression does not evaluate to boolean.")
		self.block.tipe(tm)
...
class IfStatement( Statement ):
	...
	def tipe(self, tm):
		if self.expression.tipe(tm) != "boolean":
			error("Type Error: If statement expression does not evaluate to boolean.")
...
class BinaryExpr( Expression ):
	...
	def tipe(self, tm):
		leftType = self.left.tipe(tm)
		rightType = self.right.tipe(tm)
		if leftType != rightType:
			error("Type Error: Illegal operation between two different types.")
		if self.op in ['+','-','*','/'] and leftType == "number":
			return "number"
...
class String( Expression ):
	...
	def tipe(self, tm):
		error("Type Error: String types not allowed!")
...
def checkTypes(statementList):
	tm = {} #Instantiation of the type map
